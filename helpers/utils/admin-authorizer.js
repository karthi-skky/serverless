...
const { JWT_ADMIN_SECRET, region } = require('../../config/config.json');
const { getAdmin } = require('../admin/get');
...
const handler = async (event, context) => {
  ...
  try {
    ...
    const decoded = await verifyAsync(token, JWT_ADMIN_SECRET);
    const user = await getAdmin({ login_id: decoded.sub });
